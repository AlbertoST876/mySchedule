            ...
            <div class="mx-24 my-12">
                ...
                <div class="my-8">
                    ...
                    <form action="{{ route("settings.update") }}" method="post">
                        ...
                        @foreach ($categories as $category)
                            <div class="mb-4">
                                <label class="block mb-2 text-sm font-medium text-gray-900" for="name">{{ $category -> name }}:</label>
                                <input class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full md:w-3/4 lg:w-2/4 xl:w-1/4 p-1" type="color" name="{{ $category -> id }}" value="{{ $category -> color }}" required>
                            </div>
                        @endforeach

                        @include("layouts.error", ["error" => "errorColors"])

                        <input class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none" type="submit" name="colors" value="Editar">
                    </form>
                </div>
                ...
                <div class="my-8">
                    ...
                    <form action="{{ route("settings.update") }}" method="post" enctype="multipart/form-data">
                        ...
                        <input class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none" type="submit" name="image" value="Cambiar">
                    </form>
                </div>
            </div>
        </main>
