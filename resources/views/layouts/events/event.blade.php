<div class="m-6 p-8 rounded-xl" data-color="{{ is_null($event -> color) ? $event -> categoryColor : $event -> color }}">
    ...
    <div class="flex">
        ...
        <button data-modal-target="deleteModal-{{ $i }}" data-modal-toggle="deleteModal-{{ $i }}" class="mr-2 my-2 px-5 py-3 focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm">@lang("app.delete")</button>

        <div id="deleteModal-{{ $i }}" tabindex="-1" aria-hidden="true" class="p-4 fixed top-0 left-0 right-0 z-50 hidden w-full overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <form action="{{ route("events.destroy") }}" method="post">
                        ...
                        <div class="p-4 flex items-center space-x-2 border-t border-gray-200 rounded-b">
                            ...
                            <button type="button" class="px-5 py-3 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm text-center" data-modal-hide="deleteModal-{{ $i }}">@lang("app.cancel")</button>
                        </div>
                    </form>
                </div>
            </div>
