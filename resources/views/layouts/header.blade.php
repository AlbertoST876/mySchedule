    ...
    <nav class="px-4 md:px-8 lg:px-12 py-6 bg-slate-100 border-gray-200 rounded">
        <div class="flex flex-wrap items-center justify-between">
            <a class="flex items-center" href="{{ route("index") }}">
                <img class="mr-3 w-12 h-12" src="{{ asset("icon.svg") }}" alt="@lang("messages.alt_appIcon")">
                <span class="self-center text-lg lg:text-xl font-semibold">@lang("messages.appName")</span>
            </a>
            ...
            <div class="flex items-center md:order-2">
                <button type="button" class="flex mr-3 md:mr-0 bg-slate-100 rounded-full focus:ring-4 focus:ring-gray-300" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom">
                    <span class="sr-only">@lang("messages.open_user_menu")</span>

                    @auth
                        <img class="w-12 h-12 rounded-full" src="{{ is_null(auth() -> user() -> profileImg) ? asset("./storage/img/default-user.png") : asset(auth() -> user() -> profileImg) }}" alt="@lang("messages.alt_profileImg")">
                    @else
                        <img class="w-12 h-12 rounded-full" src="{{ asset("./storage/img/default-user.png") }}" alt="@lang("messages.alt_profileImg")">
                    @endauth
                </button>
                ...
                <div class="m-4 z-50 hidden text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-md" id="user-dropdown">
                    @auth
                        ...
                        <ul aria-labelledby="user-menu-button">
                            <li><a href="{{ route("settings") }}" class="block mt-2 px-4 py-2 w-full text-left text-sm md:text-md text-gray-700 hover:bg-gray-100">@lang("messages.settings")</a></li>

                            <li>
                                <form action="{{ route("logout") }}" method="post">
                                    @csrf

                                    <input class="block mb-2 px-4 py-2 w-full text-left text-sm md:text-md text-gray-700 hover:bg-gray-100" type="submit" value="@lang("messages.logout")" onclick="return confirm('@lang("messages.logout_confirm")')">
                                </form>
                            </li>
                        </ul>
                    @else
                        <ul aria-labelledby="user-menu-button">
                            <li><a href="{{ route("login") }}" class="block mt-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">@lang("messages.login")</a></li>
                            <li><a href="{{ route("register") }}" class="block mb-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">@lang("messages.register")</a></li>
                        </ul>
                    @endauth
                </div>
                ...
                <button data-collapse-toggle="mobile-menu-2" type="button" class="inline-flex ml-1 p-2 items-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200" aria-controls="mobile-menu-2" aria-expanded="false">
                    <span class="sr-only">@lang("messages.open_menu")</span>
