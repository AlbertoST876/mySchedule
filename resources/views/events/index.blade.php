...
<html lang="{{ app() -> getLocale() }}">
    @include("layouts.head", ["title" => __("messages.events")])
            ...
            <div class="mx-6 my-12 lg:mx-24 lg:my-12">
                ...
                <div class="my-8">
                    @if (count($prevEvents) > 0)
                        @include("layouts.events.eventsList", ["id" => "prevEvents", "name" => __("messages.prevEvents"), "events" => $prevEvents])
                    @endif

                    @if (count($nextEvents) > 0)
                        @include("layouts.events.eventsList", ["id" => "nextEvents", "name" => __("messages.nextEvents"), "events" => $nextEvents])
                    @endif
                </div>
                ...
                <div class="fixed right-6 bottom-6 group">
                    <button data-modal-target="defaultModal" data-modal-toggle="defaultModal" class="flex items-center justify-center text-white bg-blue-700 rounded-full w-14 h-14 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 focus:outline-none">
                        ...
                        <span class="sr-only">@lang("messages.create_event")</span>
                    </button>
                </div>
                    ...
                    <div class="relative w-full h-full max-w-2xl md:h-auto">
                        <div class="relative bg-white rounded-lg shadow">
                            <div class="p-4 flex items-start justify-between border-b rounded-t">
                                <h3 class="text-xl font-semibold text-gray-900">@lang("messages.create_event")</h3>
                                ...
                                <button type="button" class="p-2 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm ml-auto inline-flex items-center" data-modal-hide="defaultModal">
                                    ...
                                    <span class="sr-only">@lang("messages.close_create_event")</span>
                                </button>
                            </div>
                            ...
                            <form action="{{ route("events.create") }}" method="post">
                                ...
                                <div class="p-4 flex items-center space-x-2 border-t border-gray-200 rounded-b">
                                    <input class="px-5 py-3 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm text-center" type="submit" name="create" value="@lang("messages.create")">
                                </div>
                            </form>
                        </div>
                    </div>
